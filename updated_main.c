...
int jugar(tipoJuego * juego, jugador jugadores[JUGADORES]){
    ...
    return ganador; //lo puse para que no joda mas, REVISAR ESTO
}
...
int matrizDsdArchivo(tipoJuego * juego){
    ...
    sprintf(nombreArchivo, "/home/dn/Desktop/TPE/%dx%d", juego->tablero.n,juego->tablero.n );//Cambie el path para probar en clion
    char ** aux=NULL;
    FILE * archivo;
...
void leer_movimiento(movimiento_t * mov, tipoJuego * juego)
{
    char ultimo_caracter, comando[MAXLARGOARCHIVO+MAXNPALABRA+1], nombre_archivo[256];;
    ...
    do
    {
        printf("Comando: ");
        ultimo_caracter = 0;
        fgets(comando, MAXLARGOARCHIVO+MAXNPALABRA+1, stdin);
        //if(getchar()=) REVISAAAAAAAARRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
            n = sscanf(comando,"[%d,%d] [%d,%d]%c", &mov->origen.x, &mov->origen.y, &mov->destino.x, &mov->destino.y, &ultimo_caracter);
        if(n == 5 && ultimo_caracter == '\n')
            flag_error = validarMovimiento(mov, juego->tablero);
        else if(strncmp(comando, "savegame ", 9))
        {
            for(i = 9, k = 0 ; comando[i] != '\n' && i < 267; i++, k++)
                nombre_archivo[k] = comando[i];
            nombre_archivo[++k] = '\0';
            if(i == 267)
            {
                flag_error = 3;
...
int validarMovimiento(movimiento_t * mov, matriz_t tablero)
{
    ...
    punto_t direccion;
    calcularDireccion(*mov, &direccion);
    if (!flag && esMovimientoValido(tablero, *mov, direccion, condMovimientoJugador))
        flag = 8;
    return flag;
}
